...
(function ($) {
	$.fn.extend({
		jGravity: function (options) {
			...
			return this.each(function (index, containerElement) {
				var o = options;
...
// variables
				...
				var delta = [0, 0];
				var stage = [this.offsetWidth, this.offsetHeight, this.width, this.height];
				var isRunning = false;
...
// run()
				function run() {
					if (!isRunning) {
						isRunning = true;
						loopInt = requestAnimationFrame(loop); // weight setting
						setTimeout(function () {
							window.cancelAnimationFrame(loopInt);
							$('.welcome').html('Also so bekommst du sicher kein Geld vo dire Mueter! Du kannst es gerne noch einmal versuchen.<br><a href="index.html" class="button">Neuer Versuch</a>').show();
						}, 6000);
					}
				}
...
// loop()
				function loop() {
					...
					loopInt = requestAnimationFrame(loop);
				}
				...
				function findPos(obj) {
					...
					if (obj.offsetParent) {
						do {
							...
							if (obj.offsetParent == containerElement) {
								break;
							}
...
// getBrowserDimensions()
				function getBrowserDimensions() {
					...
					if (stage[0] != window.screenX) {
						...
						stage[0] = containerElement.offsetWidth;
						changed = true;
					}
					
					if (stage[1] != window.screenY) {
						//delta[1] = (window.screenY - stage[1]) * 50;
						stage[1] = containerElement.offsetHeight;
						changed = true;
					}
					
					if (stage[2] != window.innerWidth) {
						stage[2] = containerElement.clientWidth;
						changed = true;
					}
					
					if (stage[3] != window.innerHeight) {
						stage[3] = containerElement.clientHeight;
						changed = true;
					}
